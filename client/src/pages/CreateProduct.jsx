  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.imageUrls.length < 1)
        return setErrorMessage('You must upload at least one image');
      setLoading(true);
      setErrorMessage(false);
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
          <select name="category" id="category" onChange={handleChange}  className='border p-3 rounded-lg'>
            <option value=""></option>
            <option value="diary & egg">Diary & Eggs</option>
            <option value="flour & flake">Flour & Flakes</option>
            <option value="grain & pulse">Grains & Pulses</option>
